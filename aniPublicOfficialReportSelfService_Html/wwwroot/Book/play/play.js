const books={
    play:{
        ...
        ,render:{
            ...
            ,setMode:()=>{const R=books.play.render,pl=books.play.id(R.mode),z=books.play.sem.Z,u=R.el.lvBars.querySelector('button[data-zm]');R.el.lvBars.querySelectorAll('button').forEach(x=>x.classList.toggle('on',+x.dataset.lv===R.mode));if(u)u.disabled=(z.on&&z.inv)?!books.play.child(pl).length:!books.play.up[pl];} // ⬆ is enabled by the end it now leads to: a parent, or – turned round – a child
            ,bar:()=>{const lv=books.play.render.lv,cld=books.play.child,ix=books.play.ix,sw=books.play.sem.Z.on&&books.play.sem.Z.inv,btn=p=>{const o=lv[ix(p)]||{pl:p,nm:p,ic:'•'};return '<button data-lv="'+ix(p)+'" title="'+o.pl+' '+o.nm+'">'+o.ic+'</button>';},td1=p=>'<td rowspan="2">'+btn(p)+'</td>',tdx=p=>'<td>'+btn(p)+'</td>',chain=p=>{const a=[p];let c=cld(p);while(c.length===1){a.push(c[0]);c=cld(c[0]);}return a;};let node='pl0',cc=cld(node);while(cc.length===1){node=cc[0];cc=cld(node);}const spine=books.play.path(node),cols=cld(node).slice().reverse().map(chain);books.play.render.el.lvBars.innerHTML='<table><tr>'+spine.map(td1).join('')+(cols[0]||[]).map(p=>'<td class="txt">'+btn(p)+'</td>').join('')+'<td rowspan="2" class="zm"><button data-zm="up" title="'+(sw?'into the detail':'coarser')+'">\u{1F446}</button></td></tr><tr>'+(cols[1]||[]).map(p=>'<td class="pag">'+btn(p)+'</td>').join('')+'</tr></table>';books.play.render.el.lvBars.onclick=ev=>{const x=ev.target.closest('button');if(!x)return;if(x.dataset.zm==='up'){const z=books.play.sem.Z,p=books.play.up[books.play.id(books.play.render.mode)];if(z.on&&z.inv)z.drill(1);else if(p)books.play.render.go(books.play.ix(p));}else if(x.dataset.lv!==undefined){books.play.render.go(+x.dataset.lv);}};books.play.render.setMode();}
            ...
            ,ctl:()=>{const c=lvCtl,pl=books.play.id(books.play.render.mode),kids=books.play.child(pl),sw=books.play.sem.Z.on&&books.play.sem.Z.inv,p=books.play.up[pl];let h='';if(sw)h='<button data-go="up" title="coarser"'+(p?'':' disabled')+'>\u{1F447}</button>';else kids.slice().reverse().forEach(k=>{const K=books.play.LV[books.play.ix(k)];h+='<button data-go="'+books.play.ix(k)+'" title="finer: '+(K?K.t:k)+'">\u{1F447}</button>';});c.innerHTML=h;c.onclick=ev=>{const x=ev.target.closest('button');if(!x||x.dataset.go===undefined)return;if(x.dataset.go==='up'){const p=books.play.up[pl];if(p)books.play.render.go(books.play.ix(p));}else books.play.render.go(+x.dataset.go);};}
        ...
        ,sem:{
            Z:{
                ...
                ,zb:()=>{const on=books.play.sem.Z.on;zmT.textContent=on?'\u2299':'\u25CE';zmT.title=on?'Collapse the word map back into its band':'Expand the word map over the reading area';} // ◎ when the map is down and ⊙ when it is up: the switch in the menu bar, where it stands the same however the map is shaped
                ,ib:()=>{const i=books.play.sem.Z.inv;zmI.classList.toggle('on',!!i);zmI.title='Zoom: '+(i?'up':'down')+' means into the detail – the two-finger pinch and ↑/↓ alike. Click to turn it round.';
                    if(books.play.sem.Z.on){books.play.render.bar();books.play.render.ctl();}} // the two buttons take their new roles at once – ⬆ into the detail, ⬇ coarser – so what stands there is never a lie // the ⇄ switch: the same glyph either way, only pressed or not – the title is what says which way up and down now read
                ,show:()=>{const z=books.play.sem.Z,R=books.play.render,E=R.el;if(z.on)return;z.on=1;z.box().style.display='block';document.body.classList.add('zoom'); // the overlay covers the screen itself (inset:-50vmax) – no rect maths to be out-zoomed
                    R.bar();R.ctl(); // the ⬆/⬇ buttons take the roles ⇄ gives them while the map is up
                    [E.prev,E.next,E.lvBars.querySelector('button[data-zm]'),lvCtl].forEach(el=>R.blink(el,3)); // the two hands and both ways out of the level blink three times, to say where they are
                ...
                ,hide:()=>{const z=books.play.sem.Z;if(!z.on)return;z.on=0; // going out of the zoom only puts the map away: what the pointer happens to rest on is not opened – a click, or ↓ on it, is what chooses a node
                    ...
                    z.nav.redraw();z.zb();books.play.render.bar();books.play.render.ctl();} // the band is narrow and set in smaller type, so the areas are drawn and measured again – and the buttons go back to their plain reading
                ...
                ,init:()=>{
                    ...
                    zmT.onclick=()=>{z.on?z.hide():z.show();};z.zb(); // the switch in the menu bar works the same two ways the keys do
                    try{z.inv=localStorage.getItem(z.invKey)==='1'?1:0;}catch(e){} // the way round the reader chose is theirs across visits too
                    zmI.onclick=()=>{z.inv=z.inv?0:1;try{localStorage.setItem(z.invKey,z.inv?'1':'0');}catch(e){}z.ib();};z.ib(); // ⇄ turns a gesture round – and, while the map is up, the ↑/↓ buttons with it; a key keeps its own reading
