const books={
    play:{
        ...
        ,render:{
            ...
            ,setMode:()=>{const R=books.play.render,pl=books.play.id(R.mode),z=books.play.sem.Z,u=R.el.lvBars.querySelector('button[data-zm]');R.el.lvBars.querySelectorAll('button').forEach(x=>x.classList.toggle('on',+x.dataset.lv===R.mode));if(u)u.disabled=(z.on&&z.inv)?!books.play.child(pl).length:!books.play.up[pl];} // what disables the ⬆/⬇ button is the end it leads to – with the map up and ⇄ turned round, that end is the finer one
            ,bar:()=>{const lv=books.play.render.lv,cld=books.play.child,ix=books.play.ix,sw=books.play.sem.Z.on&&books.play.sem.Z.inv,btn=p=>{const o=lv[ix(p)]||{pl:p,nm:p,ic:'•'};return '<button data-lv="'+ix(p)+'" title="'+o.pl+' '+o.nm+'">'+o.ic+'</button>';},td1=p=>'<td rowspan="2">'+btn(p)+'</td>',tdx=p=>'<td>'+btn(p)+'</td>',chain=p=>{const a=[p];let c=cld(p);while(c.length===1){a.push(c[0]);c=cld(c[0]);}return a;};let node='pl0',cc=cld(node);while(cc.length===1){node=cc[0];cc=cld(node);}const spine=books.play.path(node),cols=cld(node).slice().reverse().map(chain);books.play.render.el.lvBars.innerHTML='<table><tr>'+spine.map(td1).join('')+(cols[0]||[]).map(p=>'<td class="txt">'+btn(p)+'</td>').join('')+'<td rowspan="2" class="zm"><button data-zm="'+(sw?'down':'up')+'" title="'+(sw?'finer':'coarser')+'">'+(sw?'\u{1F447}':'\u{1F446}')+'</button></td></tr><tr>'+(cols[1]||[]).map(p=>'<td class="pag">'+btn(p)+'</td>').join('')+'</tr></table>';books.play.render.el.lvBars.onclick=ev=>{const x=ev.target.closest('button');if(!x)return;if(x.dataset.zm==='down')books.play.sem.Z.drill(1);else if(x.dataset.zm==='up'){const p=books.play.up[books.play.id(books.play.render.mode)];if(p)books.play.render.go(books.play.ix(p));}else if(x.dataset.lv!==undefined){books.play.render.go(+x.dataset.lv);}};books.play.render.setMode();}
        ...
        ,sem:{
            Z:{
                ...
                ,ib:()=>{const i=books.play.sem.Z.inv // the ⇄ switch: the same glyph either way, only pressed or not – the title is what says which way the gestures now read, since that is the whole question
                    ...
                    if(books.play.sem.Z.on){books.play.render.bar();books.play.render.ctl();}} // turned round while the map is up → the ↑/↓ buttons turn with it
                ,show:()=>{const z=books.play.sem.Z,R=books.play.render,E=R.el;if(z.on)return;z.on=1;z.box().style.display='block';document.body.classList.add('zoom'); // the overlay covers the screen itself (inset:-50vmax) – no rect maths to be out-zoomed
                    R.bar();R.ctl(); // the ↑/↓ buttons take the pinch's reading while the map is up (⇄)
                ...
                ,hide:()=>{const z=books.play.sem.Z;if(!z.on)return;z.on=0; // going out of the zoom only puts the map away: what the pointer happens to rest on is not opened – a click, or ↓ on it, is what chooses a node
                    ...
                    z.nav.redraw();z.zb();books.play.render.bar();books.play.render.ctl();} // the band is narrow and set in smaller type, so the areas are drawn and measured again – and the ↑/↓ buttons go back to the keyboard's reading
