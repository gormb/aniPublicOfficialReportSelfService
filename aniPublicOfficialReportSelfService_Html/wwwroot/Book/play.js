const books={
    play:{
        ...
        // Språkbytte: toggler mellom NO/EN (bruker md.set)
        lang:()=>{
            const no=/NO_/.test(books.play.md.fn); // nåværende språk
            books.play.md.set(no?'b/LifeDemandedDeath/b_EN_FREE.md':'b/LifeDemandedDeath/b_NO_FREE.md');
        ...
        render:{
            ...
            toc:()=>{
                ...
                let o=[];
                if(m===3){let l=0;p.pages.forEach((pg,i)=>{if(pg.h){l=pg.h[0]===2?0:1;o.push(a(pg.h[1],i+1,l,ic[pg.h[0]]||'📄'));}else if(pg.pn)o.push(a('p.'+pg.pn,i+1,l+1,'📄'));});}
                else if(m===1)o=p.chs.map((ch,i)=>a(ch[0].h[1],i,0,'📖'));
                else if(m===2)o=p.subs.map((su,i)=>a(su[0].h[1],i,su[0].h[0]===2?0:1,ic[su[0].h[0]]||'📑'));
                // m===0 (hele boka): ingen navigasjon
            ...
            sent:s=>/[.!?\u2026]["'\u201D\u2019\u00BB]?$/.test(s.trim()),
            head:p=>{const r=books.play.render,t=p.h&&p.h[1];if(!t)return '';const k=p.h[0],s=r.slug(t);return '<'+(k===2?'h2':'h3')+(s?' id="'+s+'"':'')+'>'+r.esc(t)+'</'+(k===2?'h2':'h3')+'>';},
...
dbNav.addEventListener('click',ev=>{
    ...
    if(a){const r=books.play.render;r.mode=+a.dataset.m;r.idx=+a.dataset.i;r.setMode();r.toc();r.draw();}
});
