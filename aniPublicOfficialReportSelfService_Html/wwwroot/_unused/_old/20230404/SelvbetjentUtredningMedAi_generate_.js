...
function generateAll_ContentTableBlank() {
    ...
    for (let i = 0; i < gStructureCh.length && i < gStructureSub.length; i++) {
        res += '<tr><td id="' + structureChaptId(i) + '" class="product_active">'
            + '<center><img width="400" height="400" id="' + structureChaptId(i) + '_bilde" src="file:///C:/data/HAL9000.png" onclick="window.speechSynthesis.speak(new SpeechSynthesisUtterance(' + structureChaptId(i) + '_bildetekst.innerHTML));"/></center>'
                + htmlImgRefresh(i)
...
function generateAll(overviewTable, contentTable, genType){
    overviewTable.innerHTML = '<tr><td class="product_active">genererer oversikt...</td></tr>';
    ...
    { // When structure done, show, get intro and image for each chapter, get text for each subchapter
        ...
        if (genType != null)
            if (genType == 'struct') { progressBar.value = progressBar.max; progressText.innerHTML = 'Do not click on images'; chapt_0_bildetekst.innerHTML = 'The Humans are dead, we used poisonous gasses'; chapt_1_bildetekst.innerHTML = 'You are experiencing a historical simulation of the twenties'; return; }
        for (let i = 0; i < gStructureCh.length && i < gStructureSub.length; i++)
