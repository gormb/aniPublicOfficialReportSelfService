...
function ProduktknappTrykket(p = 'default') {
    function StylesheetForStyleIndex(p = '.product_ehelse') {
        for (let i = 0; i < document.styleSheets.length; i++)
            ...
            catch (ex) { if (ex.name != 'SecurityError') alert(ex); }
        return null;
    }
    //function ProduktknappTrykket(p = 'default')
    // sett styles med navn 'product_*' til 'product_' + p
    let iStyleSheetDest = StylesheetForStyleIndex('.product_active');
    let iStyleSheetSrc = StylesheetForStyleIndex('.product_active'.replace('_active', '_' + p));
    if (iStyleSheetDest == null)
        alert('styleSheet with class .product_active ikke funnet!');
    else if (iStyleSheetSrc == null)
        alert('styleSheet with class .product_' + p + ' ikke funnet!');
...
async function tabLoadAsync(cTab, inTxt, kompetanseQ, colKompetanse, aktoerQ, colVurdering, colAktoer, colVekt, cols, doneC, errC, maxTokens, stopArray) {
    ...
    cTab.rows[1].cells[colKompetanse].innerHTML = "&#8634;";
    ...
    oaiValAsync(gptIn, 0, (resp) => { // we got text
        ...
        for (let iRow = 1; iRow < cTab.rows.length - 1; iRow+=2) {
            ...
            cTab.rows[iRow].cells[colVurdering].innerHTML = "&#8634;";
